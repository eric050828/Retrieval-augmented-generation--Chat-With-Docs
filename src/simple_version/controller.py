...
class MainController:
    ...
    def upload_file(
        ...
        self._report("Splitting data")
        chunks = self.db_model.split(data)

        self._report("Storing chunks")
        self.db_model.store(collection_name, chunks)
        self._report("Upload completed")
    ...
    def get_response(
        ...
        self._report("generating response...")
        start = time.time()
        response = self.rag_model.generate_response(collection_name, query)
        end = time.time()
        self._report(f"response generated, spent {end - start:.4f}s")
        return response
